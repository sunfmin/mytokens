...
private func usage() -> CommandResult {
    ...
    usage:
      ...
      mytokens get <service> [--account <label>]
      mytokens list
...
private func runAdd(_ argv: [String], _ deps: Dependencies) -> CommandResult {
    ...
    guard let value = deps.input.promptForSecret(service: service, account: account) else {
        return fail("cancelled; nothing stored\n", 1)
    }
    guard !value.isEmpty else { return fail("empty value; nothing stored\n", 1) }
    do {
        try deps.store.upsert(service: service, account: account, value: value, kind: kind, meta: meta)
...
private func runGet(_ argv: [String], _ deps: Dependencies) -> CommandResult {
    ...
    let account = args.flags["account"] ?? "default"
    do {
        if let value = try deps.store.get(service: service, account: account) {
            return CommandResult(stdout: value, stderr: "", exitCode: 0)  // exact value, no newline
        }
...
private func runList(_ argv: [String], _ deps: Dependencies) -> CommandResult {
    do {
        ...
        let rows = records.map { r -> String in
            let meta = r.meta.map { "  " + $0 } ?? ""
            return "\(r.service)/\(r.account)\t\(r.kind)\(meta)"
        }
...
private func runSelftest() -> CommandResult {
    ...
    do {
        try store.upsert(service: service, account: account, value: "SELFTEST-OK", kind: "static", meta: ["probe": true])
        let roundtripped = try store.get(service: service, account: account) == "SELFTEST-OK"
